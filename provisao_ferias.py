...
def lancar_folha_ferias_salario(data) :
    ...
    with open('log_provisoes.txt', 'a') as log:
        with open('Relatorios_Funcionarios_Provisoes_Provisao_Ferias_Grafica.csv', 'r') as folha_13:
                ...
                if linha:
                    ...
                    if funcionario:
                        ...
                        if linha[0].upper() == 'SALDO ANTERIOR':
                            saldo_anterior = converter_string_para_float(linha[1]) + converter_string_para_float(linha[2])
                            # print(linha[1], linha[2])
                            # print(f'saldo anterior ********* {saldo_anterior}')
                            saldo_anterior_INSS = linha[3]
                            saldo_anterior_FGTS = linha[4]
                            saldo_anterior_boleano = True
                            # print(f'saldo anterior => {saldo_anterior}')
                        if linha[0].upper() == 'SALDO':
                            saldo = converter_string_para_float(linha[1]) + converter_string_para_float(linha[2])
                            saldo_INSS = linha[3]
                            saldo_FGTS = linha[4]
                            saldo_boleano = True
                            # print(f'saldo => {saldo}')
                        if linha[0].upper() == 'PAGO':
                            pago = converter_string_para_float(linha[1]) + converter_string_para_float(linha[2])
                            pago_INSS = linha[3]
                            pago_FGTS = linha[4]
                            pago_boleano = True
                            # print(f'pago => {pago}')
                    ...
                    if saldo_anterior_boleano and saldo_boleano and pago_boleano and funcionario:
                        ...
                        if dic_func.get(nome_funcionario):
                            ...
                            if provisao > 0:
                                ...
                                prov_deb = dic_func.get(nome_funcionario)['prov_ferias_deb']
                                prov_cred = dic_func.get(nome_funcionario)['prov_ferias_cred']
                                prov_hist = dic_func.get(nome_funcionario)['prov_ferias_hist']
                                ...
                                provisao *= -1
                                prov_cred = dic_func.get(nome_funcionario)['prov_ferias_deb']
                                prov_deb = dic_func.get(nome_funcionario)['prov_ferias_cred']
                                prov_hist = dic_func.get(nome_funcionario)['prov_ferias_hist']
                                # print(nome_funcionario, prov_deb, prov_cred, round(provisao, 2))
                                gerar_lancamentos_13_salario(prov_deb, prov_cred, round(provisao, 2), prov_hist, data)

                            if provisao_13_INSS > 0:
                                prov_deb_INSS = dic_func.get(nome_funcionario)['inss_ferias_deb']
                                prov_cred_INSS = dic_func.get(nome_funcionario)['inss_ferias_cred']
                                prov_hist_INSS = dic_func.get(nome_funcionario)['inss_ferias_hist']
                                ...
                                provisao_13_INSS *= -1
                                prov_cred_INSS = dic_func.get(nome_funcionario)['inss_ferias_deb']
                                prov_deb_INSS = dic_func.get(nome_funcionario)['inss_ferias_cred']
                                prov_hist_INSS = dic_func.get(nome_funcionario)['inss_ferias_hist']
                                gerar_lancamentos_13_salario(prov_deb_INSS, prov_cred_INSS, round(provisao_13_INSS, 2), prov_hist_INSS, data)
                            if pago_INSS > 0:
                                prov_cred_INSS = dic_func.get(nome_funcionario)['inss_ferias_deb']
                                prov_deb_INSS = dic_func.get(nome_funcionario)['inss_ferias_cred']
                                prov_hist_INSS = dic_func.get(nome_funcionario)['inss_ferias_hist_bx']
                                gerar_lancamentos_13_salario(prov_deb_INSS, prov_cred_INSS, round(pago_INSS, 2), prov_hist_INSS, data)

                            if provisao_13_FGTS > 0:
                                prov_deb_FGTS = dic_func.get(nome_funcionario)['fgts_ferias_deb']
                                prov_cred_FGTS = dic_func.get(nome_funcionario)['fgts_ferias_cred']
                                prov_hist_FGTS = dic_func.get(nome_funcionario)['fgts_ferias_hist']
                                ...
                                provisao_13_FGTS *= -1
                                prov_cred_FGTS = dic_func.get(nome_funcionario)['fgts_ferias_deb']
                                prov_deb_FGTS = dic_func.get(nome_funcionario)['fgts_ferias_cred']
                                prov_hist_FGTS = dic_func.get(nome_funcionario)['fgts_ferias_hist']
                                gerar_lancamentos_13_salario(prov_deb_FGTS, prov_cred_FGTS, round(provisao_13_FGTS, 2), prov_hist_FGTS, data)
                            if pago_FGTS > 0:
                                prov_cred_FGTS = dic_func.get(nome_funcionario)['fgts_ferias_deb']
                                prov_deb_FGTS = dic_func.get(nome_funcionario)['fgts_ferias_cred']
                                prov_hist_FGTS = dic_func.get(nome_funcionario)['fgts_ferias_hist_bx']
